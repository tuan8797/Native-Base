...
exports.convertToDp = exports.convertRemToAbsolute = exports.convertAbsoluteToRem = exports.baseFontSize = void 0;
...
function orderedExtractInObject(parent, values) {
  return [omitUndefined(orderedPick(parent, values)), omitUndefined((0, _lodash.omit)(parent, values))];
}
/**
 *
...
function findLastValidBreakpoint(values, themeBreakpoints, currentBreakpoint) {
  ...
  return valArray[currentBreakpoint] ?? valArray.slice(0, currentBreakpoint + 1).filter(v => !(0, _lodash.isNil)(v)).pop();
}
...
const platformSpecificSpaceUnits = theme => {
  ...
  scales.forEach(key => {
    const scale = (0, _lodash.get)(theme, key, {});
    const newScale = {
      ...scale
    };
