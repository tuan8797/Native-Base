...
const SPREAD_PROP_SPECIFICITY_MAP = {
  ...PADDING_MAP,
  ...MARGIN_MAP
};
function propsSpreader(incomingProps, incomingSpecifity) {
  const flattenedDefaultProps = {
    ...incomingProps
  };
  const specificity = {};
  ...
  return (0, _lodash.merge)({}, flattenedDefaultProps);
}
...
function usePropsResolution(component, incomingProps, state, config) {
  ...
  const componentTheme = (config === null || config === void 0 ? void 0 : config.componentTheme) ?? (0, _lodash.get)(theme, `components.${component}`, {});
  if (process.env.NODE_ENV === 'development' && incomingProps.debug) {
    /* eslint-disable-next-line */
    console.log(`%c${component}`, 'background: #d97706; color: #111; font-weight: 700; padding: 2px 8px;');
    /* eslint-disable-next-line */
...
const usePropsResolutionWithComponentTheme = (componentTheme, incomingProps, state, config) => {
  ...
  if (config !== null && config !== void 0 && config.extendTheme) {
    config === null || config === void 0 ? void 0 : config.extendTheme.map(componentName => {
      extendedTheme.push((0, _lodash.get)(theme, `components.${componentName}`, {}));
    });
  }
  if (!(0, _lodash.isEmpty)(componentTheme)) extendedTheme.push(componentTheme);
  ...
  const callPropsFlattener = function () {
    ...
    return (0, _propsFlattener.propsFlattener)({
  ...
  const incomingWithDefaultProps = (0, _lodash.merge)({}, componentTheme.defaultProps || {}, cleanIncomingProps);
  ...
  if (combinedVariantStyle) {
    ...
    [flattenVariantStyle, variantSpecificityMap] = callPropsFlattener(combinedVariantStyle, baseSpecificityMap || specificityMap);
  ...
  // STEP 4: merge
  const defaultStyles = (0, _lodash.merge)({}, flattenBaseStyle, flattenVariantStyle, flattenSizeStyle);
  ...
  const resolvePseudoProps = (flatPseudoProp, baseStylePseudoProp) => {
    for (const prop in flatPseudoProp) {
      baseStylePseudoProp[prop] = flatPseudoProp[prop]; /* Replace all the similar prop from from internal props */
    }
  ...
  const defaultSpecificity = (0, _lodash.merge)({}, specificityMap, baseSpecificityMap, variantSpecificityMap, sizeSpecificityMap);
  flattenProps = propsSpreader({
    ...defaultStyles,
    ...flattenProps
  }, defaultSpecificity);
  const resolvedProps = (0, _tools.omitUndefined)({
    ...flattenProps,
    ...ignoredProps
  });
