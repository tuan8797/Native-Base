    ...
    if (_base.themePropertyMap[property]) {
      ...
      if (typeof propValues === 'string' || typeof propValues === 'number') {
        ...
        for (let nestedProp in propValues) {
          newProps[nestedProp] = (0, _lodash.get)(theme, `${_base.themePropertyMap[nestedProp]}.${propValues[nestedProp]}`, propValues[nestedProp]);
        }
...
const extractPropertyFromFunction = (property, props, theme, componentTheme) => {
  ...
  if (componentTheme && typeof componentTheme[_base.themePropertyMap[property]] === 'function') {
    ...
    propValues = isNested ? {
      ...(0, _lodash.get)(funcProps, `${props[property]}`)
    } : {
      ...funcProps
    };
  } else {
    // If the entry is any value other than function then return the whole object or value
    propValues = (0, _lodash.get)(componentTheme, `${_base.themePropertyMap[property]}.${props[property]}`);
  }
  return propValues;
};
...
function mergeUnderscoreProps(newProps, props) {
  ...
  _props.forEach(propName => {
    var _newProps$propName, _props$propName;
    // Adding color based on bg contrast if no color is given
    const bg = newProps.bg ?? newProps.backgroundColor;
    const textColor = bg ? {
      color: (0, _useContrastText.useContrastText)(bg, ((_newProps$propName = newProps[propName]) === null || _newProps$propName === void 0 ? void 0 : _newProps$propName.color) ?? ((_props$propName = props[propName]) === null || _props$propName === void 0 ? void 0 : _props$propName.color))
    } : {};
...
function calculateProps(theme, colorModeProps, componentTheme, props, windowWidth) {
  ...
  if (componentTheme) {
    ...
    newProps = (0, _lodash.mergeWith)(newProps, componentBaseStyle,
    // @ts-ignore
    ...
    const variant = props.variant || (0, _lodash.get)(componentTheme, 'defaultProps.variant');
    // Extracting props from variant
    if (variant && componentTheme.variants && componentTheme.variants[variant]) {
      const colorScheme = props.colorScheme || (0, _lodash.get)(componentTheme, 'defaultProps.colorScheme');
      ...
      newProps = (0, _lodash.mergeWith)(newProps, variantProps,
      // @ts-ignore
      (objValue, srcValue, key) => {
        if (!(0, _lodash.isNil)(objValue)) {
          delete newProps[key];
  ...
  let extractedProps = extractProps(props, theme, colorModeProps, componentTheme, currentBreakpoint);
  // added this to handle order of props
  // @ts-ignore
  newProps = (0, _lodash.mergeWith)(newProps, extractedProps, (objValue, srcValue, key) => {
    if (!(0, _lodash.isNil)(objValue)) {
      delete newProps[key];
    }
  });
