...
import { Box, Button, Pressable,
// Select,
Image, Spinner, Text, Input, Checkbox, Slider,
// Icon,
HStack, Heading } from '../../components/primitives';
...
describe('props resolution', () => {
  ...
  it('Input: Basic check', () => {
    ...
    expect(inputElementStack.props.style.marginLeft).toBe(defaultTheme.space['3']);
    expect(inputElementStack.props.style.marginRight).toBe(defaultTheme.space['3']);
  });
  it('Input: color mode', () => {
    const newTheme = extendTheme({
      config: {
  ...
  it('Pseudo props test: importatnt on Button', () => {
    ...
    expect(button.props.style.backgroundColor).toBe(defaultTheme.colors.green['400']);
  });
  it('Pseudo props test: normal prop on light and dark', () => {
    ...
    const button = getByTestId('test');
    expect(button.props.style.backgroundColor).toBe(defaultTheme.colors.amber['500']);
  });
  it('Pseudo props test: _important overrided', () => {
    ...
    const button = getByTestId('test');
    expect(button.props.style.backgroundColor).toBe(defaultTheme.colors.emerald['800']);
  });
});
