import { cloneDeep, mergeWith, isNil, get } from 'lodash';
...
export function mergeUnderscoreProps(newProps, props) {
  ...
  _props.forEach(propName => {
    ...
    const textColor = bg ? {
      color: useContrastText(bg, ((_newProps$propName = newProps[propName]) === null || _newProps$propName === void 0 ? void 0 : _newProps$propName.color) ?? ((_props$propName = props[propName]) === null || _props$propName === void 0 ? void 0 : _props$propName.color))
    } : {};
...
export function calculateProps(theme, colorModeProps, componentTheme, props, windowWidth) {
  ...
  if (componentTheme) {
    ...
    newProps = mergeWith(newProps, componentBaseStyle,
    // @ts-ignore
    ...
    if (variant && componentTheme.variants && componentTheme.variants[variant]) {
      ...
      newProps = mergeWith(newProps, variantProps,
      // @ts-ignore
      (objValue, srcValue, key) => {
        if (!isNil(objValue)) {
          delete newProps[key];
